...
def image(image):
    if image.unauthenticated_access:
        http_url = url('raw_by_user_wh', user=image.owner.user_name, hypervisor=image.hypervisor,
    ...
    return {'uuid':image.uuid,
             ...
             'size':image.size,
             'file_url':url('raw_by_user_wh', user=image.owner.user_name, hypervisor=image.hypervisor,
